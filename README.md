...
| Metric          | Score | Notes                                                   |
|-----------------|-------|---------------------------------------------------------|
| Relevance       | /5    | Does the explanation match the log evidence?             |
| Specificity     | /5    | Is it specific to this incident, not generic?            |
| Actionability   | /5    | Are remediation steps concrete and useful?               |

> Run with `--eval` flag or check "Run quality eval" in the dashboard to score. Scores are generated by LLM-as-judge — see [the blog post](https://mister-raggs.github.io/ai/building-flare-llm-powered-incident-detection/) for methodology.
